...
class GeSHiExtended extends GeSHi {  
  ...
  private static $classShortcuts =
    'kw KEYWORDS co COMMENTS es ESCAPE_CHAR br BRACKETS st STRINGS nu NUMBERS me METHODS sy SYMBOLS sc SCRIPT re REGEXPS' ;
...
class SourceCode {
  ...
  */
  ...
  protected function getGeSHI() {
    if (!isset($this->geshi)) {
      $geshi = new GeSHi($this->plainSourceCode, $this->geshiLanguageCode);
  ...
  public static function generateHTML($file,$language,$directory,$fragmentSpecs=null,$sourceid=null) {
    $text = file_get_contents($file) ;
    $source = new SourceCode($text,$language,$sourceid) ;
    $htmlBody = $source->getHTML() ;
    $outputfilename = $directory.'/'.basename($file) ;
    
    // generate the main file
    $simpleHeader = $source->getHTMLHeader() ;
    ...
    if (isset($fragmentSpecs)) {
      ...
      foreach($fragmentSpecs as $fragmentName => $fragmentSpec) {
        $header = $source->getHTMLHeader($fragmentSpec,'background:#ffffaa ;') ;
